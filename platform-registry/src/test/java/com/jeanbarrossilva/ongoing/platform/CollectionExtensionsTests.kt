...
import com.jeanbarrossilva.ongoing.platform.registry.extensions.map
...
internal class CollectionExtensionsTests {
    @Test
    fun `GIVEN a collection WHEN mapping it to a flow THEN it's up-to-date when emitted to`() {
        ...
        val mappedFlow =
            listOf(1, 2, 3).map({ current, replacement -> current == 2 && replacement == 4 }) {
                transformationFlow
            }
        ...
        runTest {
            assertEquals(listOf(4, 6), mappedFlow.first())
        }
    }
}